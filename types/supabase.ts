...
export interface DiagnosisSession {
  id: string; // UUID
  user_id: string | null; // UUID -> profiles.id (可选，支持访客模式)
  ...
  report_content: string | null; // AI生成的完整Markdown报告
  created_at: string; // TIMESTAMPTZ
  updated_at: string; // TIMESTAMPTZ
}

export interface DiagnosisSessionInsert extends Omit<DiagnosisSession, 'id' | 'created_at' | 'updated_at' | 'user_id'> {
  id?: string;
